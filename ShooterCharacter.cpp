...
#define COLLISION_PROJECTILE ECollisionChannel::ECC_EngineTraceChannel1
#define COLLISION_WEAPON ECollisionChannel::ECC_EngineTraceChannel2
...
bool AShooterCharacter::IsEnemyFor(AController* TestPC) const
{
	...
	{
		const ARobotZombieGameMode* DefGame = GetWorld()->GameState->GameModeClass->GetDefaultObject<ARobotZombieGameMode>();
...
float AShooterCharacter::TakeDamage(float Damage, struct FDamageEvent const& DamageEvent, class AController* EventInstigator, class AActor* DamageCauser)
{
	...
	ARobotZombieGameMode* const Game = GetWorld()->GetAuthGameMode<ARobotZombieGameMode>();
...
bool AShooterCharacter::Die(float KillingDamage, FDamageEvent const& DamageEvent, AController* Killer, AActor* DamageCauser)
{
	...
	GetWorld()->GetAuthGameMode<ARobotZombieGameMode>()->Killed(Killer, KilledPlayer, this, DamageType);
...
void AShooterCharacter::OnDeath(float KillingDamage, struct FDamageEvent const& DamageEvent, class APawn* PawnInstigator, class AActor* DamageCauser)
{
	...
	if (DeathAnimDuration > 0.f)
	{
		...
		GetWorldTimerManager().SetTimer(TimerHandle, this, &AShooterCharacter::SetRagdollPhysics, FMath::Max(0.1f, DeathAnimDuration - 1.0f), false);
	}
	else
	{
		SetRagdollPhysics();
...
void AShooterCharacter::UpdateCameraThirdPerson(const FVector & CameraLocation, const FRotator & CameraRotation)
{
	if (Controller && ThirdPersonCamera)
	{
		...
		//GEngine->AddOnScreenDebugMessage(-1, 1.0, FColor::Red, FString::SanitizeFloat(ThirdPersonCamera->FieldOfView));
		//GEngine->AddOnScreenDebugMessage(-1, 1.0, FColor::White, FString::Printf(TEXT("Bool: %s"), bIsTargeting ? TEXT("true") : TEXT("false")));
	}
}
...
bool AShooterCharacter::IsFirstPerson() const
{
	...
	//GEngine->AddOnScreenDebugMessage(-1, 1.0, FColor::White, FString::Printf(TEXT("Bool: %s"), bIsThirdPerson ? TEXT("true") : TEXT("false")));
	//GEngine->AddOnScreenDebugMessage(-1, 1.0, FColor::Red, FString::SanitizeFloat(Health));
}
