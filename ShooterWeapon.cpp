...
void AShooterWeapon::UpdateMeshes() {
	if (GetPawnOwner()) {
		const bool bIsFirstPerson = GetPawnOwner()->IsFirstPerson();
		GetMesh1P()->SetOwnerNoSee(!bIsFirstPerson);
		GetMesh1P()->SetOnlyOwnerSee(!bIsFirstPerson);

		GetMesh3P()->SetOwnerNoSee(bIsFirstPerson);
		GetMesh3P()->SetOnlyOwnerSee(bIsFirstPerson);
	}
}
...
bool AShooterWeapon::HasInfiniteClip()
{
	const AShooterPlayerController* MyPC = (MyPawn != NULL) ? Cast<const AShooterPlayerController>(MyPawn->Controller) : NULL;
	return WeaponConfig.bInfiniteClip || (MyPC && MyPC->HasInfiniteClip());
}

float AShooterWeapon::GetEquipStartedTime()
{
	return EquipStartedTime;
}

float AShooterWeapon::GetEquipDuration()
{
...
float AShooterWeapon::PlayWeaponAnimation(const FWeaponAnim& Animation)
{
	...
	if (MyPawn)
	{
		...
		//GEngine->AddOnScreenDebugMessage(-1, 1.0, FColor::White, FString::Printf(TEXT("Bool: %s"), MyPawn->IsFirstPerson() ? TEXT("true") : TEXT("false")));
		if (UseAnim)
		{
